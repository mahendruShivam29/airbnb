...
export default function Dashboard(props) {
  ...
  useEffect(() => {
    ...
    const fetchFavs = async () => {
      try {
        const res = await api.get(`/traveler/favorites?t=${Date.now()}`, { withCredentials: true });
  ...
  const toggleFav = async (id, isFav) => {
    try {
      if (isFav) await api.delete(`/traveler/favorites/${id}`);
      else await api.post(`/traveler/favorites/${id}`);
